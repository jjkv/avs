...
let rec print_results o = function
  | [] -> ()
  | [r] -> print_result o r
  | r::rs -> Printf.fprintf o "%a\n%a" print_result r print_results r

exception TheRoof of string
let error_out s = raise @@ TheRoof s
...
let inv3 b:bvec = 
    ...
    in (List.map normalize xs)

let promote = (function (`L_Int x) -> `L_Sym (inv3 @@ Bvec.bvec_of_int x) | y -> y)
        ...
        (match bvops with
             | None -> 
               let x, y = (function `L_Int x, `L_Int y -> x, y
                                  | _ -> error_out "unimplemented mixed arith") (v1, v2)
               in `L_Int (int_op x y)
             | Some (bv_op, bv_proc) ->
               (match (v1, v2) with
                    ...
                    | (       _,        _) ->  error_out "bad args for arith")) in
    (match s with 
      | [] -> error_out "empty stack" 
      | ((cid, cpc, regs)::rest) ->
        ...
        in (match (current_inst p (s, path)) with
            ...
            | I_if_zero (`L_Reg r, n) ->
              ...
              | I_ret (`L_Reg r) ->
                (match s with 
                     | [] -> error_out "empty stack again"
                     | ("main",_,_)::[] -> `Return ((s, path), Hashtbl.find regs r)
                     | _ -> error_out "catch all ret")
              ))
...
let run_prog (p:prog) (sch:scheduler) (max_steps:int):prog_ret list = 
    ...
    let rec prog_loop xs i a =
        (match xs with
          | [] -> a
          | (c::cs) ->
            let n, path = (function (((x, _, _)::_), y) -> (x,y) | _ -> error_out "run prog") c in
            if i > max_steps then (`Partial path)::a else
            (match (run_inst p c) with
                 | `Return ((_, p'), v) -> prog_loop cs                       (i + 1) ((`Full (promote v, p'))::a)
